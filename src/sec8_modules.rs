...
pub fn study_fn() {
    ...
    println!("[sec8_modules::study_fn] Enter input :");
    let _result = io::stdin().read_line(&mut buffer);
    println!("[sec8_modules::study_fn] Input : {buffer}");

    //trim - remove return at the end
    //let number = buffer.trim().parse::<i32>().unwrap();
    let number: i32 = buffer.trim().parse().unwrap();
    println!("[sec8_modules::study_fn] Number : {number}");

    let rand_number = random::<f64>();
    println!("[sec8_modules::study_fn] Random number : {rand_number}");
...
pub fn guess_number() {
    ...
    loop {
        ...
        io::stdin().read_line(&mut buffer).expect("[sec8_modules::guess_number] Invalid input !!!");
        ...
        if buffer.trim().to_uppercase() == "X" {
            ...
            let input_number: u32 = buffer.trim().parse().expect("[sec8_modules::guess_number] Invalid input number");
